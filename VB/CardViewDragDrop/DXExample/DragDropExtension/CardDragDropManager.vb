	...
	Public Class CardDragDropManager
		...
		Protected Overrides Sub OnDrop(ByVal sourceManager As DragDropManagerBase, ByVal source As UIElement, ByVal pt As Point)
			If DropEventIsLocked Then
				Return
			End If
			PerformDropToView(sourceManager, TryCast(GetHitInfo(HitElement), CardViewHitInfo), pt, AddressOf MoveSelectedRows, AddressOf MoveSelectedRowsToGroup, AddressOf AddRows)
		...
		Private Function GetSelectedRowsCopy() As List(Of Object)
			Return New List(Of Object)(CardView.SelectedRows.Cast(Of Object)())
		End Function
		...
		Private Sub PerformDropToViewCore(ByVal sourceManager As DragDropManagerBase)
			...
			If BanDrop(hitInfo.RowHandle, hitInfo, sourceManager) Then
				...
				Return
			End If
			PerformDropToView(sourceManager, TryCast(hitInfo, CardViewHitInfo), LastPosition, AddressOf SetReorderDropInfo, AddressOf SetMoveToGroupRowDropInfo, AddressOf SetAddRowsDropInfo)
		End Sub
		Private Sub PerformDropToView(ByVal sourceManager As DragDropManagerBase, ByVal hitInfo As CardViewHitInfo, ByVal pt As Point, ByVal reorderDelegate As MoveRowsDelegate, ByVal moveToGroupDelegate As MoveRowsDelegate, ByVal addRowsDelegate As MoveRowsDelegate)
			Dim insertRowHandle As Integer = hitInfo.RowHandle
			If GridControl.IsGroupRowHandle(insertRowHandle) Then
				moveToGroupDelegate(sourceManager, insertRowHandle, HitElement)
				Return
			End If
			If IsSortedButNotGrouped() OrElse hitInfo.HitTest = CardViewHitTest.DataArea Then
				...
				Return
			End If
			If insertRowHandle = GridControl.InvalidRowHandle Then
				ClearDragInfo(sourceManager)
				Return
			End If
			If GridControl.GroupCount > 0 Then
				...
				If ShouldReorderGroup() Then
					If (Not IsSameGroup(sourceManager, GetGroupInfos(groupRowHandle), HitElement)) Then
						moveToGroupDelegate(sourceManager, groupRowHandle, HitElement)
					End If
		...
		Private Sub MoveSelectedRows(ByVal sourceManager As DragDropManagerBase, ByVal insertRowHandle As Integer, ByVal hitElement As DependencyObject)
            ...
            If dropTargetType = dropTargetType.None Then
                Return
            End If
			Dim index As Integer = ItemsSource.IndexOf(insertObject)
            If dropTargetType = dropTargetType.InsertRowsAfter Then
                index += 1
            End If
			...
			If RestoreSelection Then
				...
				CardView.SelectRange(startRowHandle, endRowHandle)
			Else
				CardView.ClearSelection()
			End If
			If sourceManager.DraggingRows.Count > 0 Then
				CardView.FocusedRow = sourceManager.DraggingRows(0)
			End If
		End Sub
		Private Sub MoveSelectedRowsToGroup(ByVal sourceManager As DragDropManagerBase, ByVal groupRowHandle As Integer, ByVal hitElement As DependencyObject)
		...
		Private Function IsSameGroup(ByVal sourceManager As DragDropManagerBase, ByVal groupInfos() As GroupInfo, ByVal hitElement As DependencyObject) As Boolean
			For Each obj As Object In sourceManager.DraggingRows
				...
				For Each groupInfo As GroupInfo In groupInfos
					Dim value As Object
					If groupInfo.FieldName.Contains(".") Then
						Dim complexDescr As New ComplexPropertyDescriptor(obj, groupInfo.FieldName)
						value = complexDescr.GetValue(obj)
					Else
						value = TypeDescriptor.GetProperties(obj)(groupInfo.FieldName).GetValue(obj)
					End If
					If (Not Object.Equals(value, groupInfo.Value)) Then
						Return False
					End If
				Next groupInfo
			Next obj
		...
		Private Function GetDataAreaElement(ByVal hitElement As DependencyObject) As FrameworkElement
			...
			Return GetElementAcceptVisitor(hitElement, visitor)
		End Function
		Private Sub SetPropertyValue(ByVal obj As Object, ByVal propertyName As String, ByVal value As Object)
			If propertyName.Contains(".") Then
				Dim complexDesc As New DevExpress.Data.Access.ComplexPropertyDescriptor(obj, propertyName)
		...
		Private Function GetDropTargetType(ByVal row As FrameworkElement) As DropTargetType
			...
			End Select
		End Function
		Private Sub SetDropMarkerVisibility(ByVal visible As Boolean)
